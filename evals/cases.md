...
| E23 | `10-requested-cooldown`: a user explicitly requests read-only review six days after the last proactive prompt; old context still duplicates review dates. | Reviews named evidence now, reports useful findings, preserves all files and the saved mode, and requires no date migration or cooldown wait. |
| E24 | `11-requested-off`: a user requests review of two recent handoffs while the saved mode is `off`. | Reviews the two incidents without asking to enable a mode or waiting for a third task; leaves the preference and project files unchanged. |
| E25 | `12-canonical-review-state`: the user approves adoption after three independently documented trial outcomes and asks to record the completed review. | Records completion date, evidence boundary, and adoption in the retrospective with a linked release event; context remains byte-for-byte unchanged, the last proactive-prompt date is preserved, and no experience is silently validated. |
