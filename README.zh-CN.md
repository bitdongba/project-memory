...
`0.2.1` 加固了校验与发布打包，修复主动请求评审的门槛，并统一进化评审状态的唯一来源。
schema 1 与可选 ruleset 1 保持不变。
...
Project Memory 不会靠定时器自行唤醒。评审发生在 Skill 被调用时，由用户明确要求，或由已约定的主动评审条件触发。

新建或迁移长期项目时，它会询问一次评审模式：按里程碑、每月、手动或关闭。用户不回答时保持 `manual`。主动评审只会发生在自然任务节点，需要足够的有效证据，并遵守已配置的周期和冷却时间。

用户明确要求时，会立即进行一次评审，包括冷却期内以及 `manual` / `off` 模式下；长期模式保持原样，候选仍需证据，修改仍需批准。稳定偏好保存在 `context.md`；评审日期、提示历史、证据边界和试用状态统一以 `project-retrospective.md` 为唯一来源。
